...
async function renderPaymentsPage() {
  ...
  // Текущий пользователь только через токен
  let curUser = {};
  let isAdmin = false;
  try {
    const token = localStorage.getItem('token');
    if (token) {
      const res = await fetch('https://eurpay.onrender.com/api/users/me', {
        headers: { 'Authorization': 'Bearer ' + token }
      });
      if (res.ok) {
        curUser = await res.json();
        isAdmin = curUser.role && curUser.role.toLowerCase() === 'admin';
      }
    }
  } catch (e) {
    curUser = {};
  ...
  // Все платежи получаем только с сервера!
...
`;
  ...
  if ((isAdmin || userCanCreate) && createPayBtn && payForm && payFormDiv && cancelEditBtn) {
    ...
    payForm.addEventListener('submit', async (e)=>{
      e.preventDefault();
      // Получаем текущего пользователя только через токен
      let curUser = {};
      ...
      try {
        ...
        if (token) {
          ...
          if (res.ok) {
            curUser = await res.json();
            isAdminUser = curUser.role && curUser.role.toLowerCase() === 'admin';
          }
        }
      } catch (e) {
        curUser = {};
      ...
      let feePercent = 0;
      if (!isAdminUser) {
        // Получаем feePercent из профиля текущего пользователя с сервера
        try {
          const token = localStorage.getItem('token');
          const res = await fetch('https://eurpay.onrender.com/api/users/me', {
            headers: { 'Authorization': 'Bearer ' + token }
          ...
          if (res.ok) {
            ...
            feePercent = userProfile.feePercent || 0;
          } else {
            feePercent = 0;
          }
        } catch (e) {
      ...
      // Прикрепление файлов (TODO: вынести в отдельный api/docs)
      const fileList = fd.getAll('paymentDocs');
      // TODO: реализовать загрузку файлов на сервер через отдельный эндпоинт (api/docs)
      // Пока только сохраняем id-шники файлов в payObj.docs и сохраняем в localStorage документы
      ...
      async function handleFile(i){
        ...
        reader.onload = async (ev)=>{
          ...
          const docName = `${payObj.orderNumber || ''} от ${payObj.date} — ${file.name}`;
          docs.push({
            id: docId,
            name: docName,
            data: dataURL,
            linkedPaymentId: payObj.id // payObj.id появится только после создания на сервере!
          });
      ...
      async function finalize(){
        try {
          ...
          if(editingIndex===null){
            ...
            const res = await fetch('https://eurpay.onrender.com/api/payments', {
              method: 'POST',
              headers: {
            ...
            // Получаем id редактируемого платежа
            // Сначала загружаем актуальный список платежей с сервера
            const paysArr = await fetchPayments();
            const sorted = paysArr.sort((a,b)=> new Date(b.date)-new Date(a.date));
            const payToEdit = sorted[editingIndex];
            if (!payToEdit) {
              showToast('Ошибка: платёж не найден','error');
              return;
            }
            // Редактирование платежа (PUT)
            const res = await fetch(`https://eurpay.onrender.com/api/payments/${encodeURIComponent(payToEdit.id)}`, {
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + token
              },
              body: JSON.stringify({...payToEdit, ...payObj, ownerEmail: curUser.email })
            });
            if (!res.ok) throw new Error('Ошибка обновления платежа');
            showToast('Платёж обновлён','success');
          }
        } catch (e) {
          showToast('Ошибка сохранения платежа','error');
        }
        payForm.reset();
        payFormDiv.style.display='none';
        editingIndex = null;
  ...
  if (isAdmin && exportCsvBtn) {
    exportCsvBtn.addEventListener('click', async ()=>{
      let paysArr = await fetchPayments();
      if(!paysArr.length){
  ...
  async function renderPaymentsList(){
    ...
    if (isAdminUser) {
      paysList.querySelectorAll('.payDelBtn').forEach(btn=>{
        btn.addEventListener('click', async function(){
          ...
          try {
            const token = localStorage.getItem('token');
            const res = await fetch(`https://eurpay.onrender.com/api/payments/${encodeURIComponent(payToDelete.id)}`, {
              method: 'DELETE',
              headers: {
                'Authorization': 'Bearer ' + token
              }
            });
            if (!res.ok) throw new Error('Ошибка удаления платежа');
            showToast('Платёж удалён','info');
          } catch (e) {
            showToast('Ошибка удаления платежа','error');
          }
    ...
    // Редактирование
    if (isAdminUser && payForm && payFormDiv && cancelEditBtn) {
  ...
  // Генерация УНП-номера (не требуется, id создаёт сервер)
...
async function showPdfAgentReport(payObj) {
  ...
  let userObj = {};
  try {
    // Получаем пользователя по email через сервер
    const token = localStorage.getItem('token');
    const res = await fetch(`https://eurpay.onrender.com/api/users?email=${encodeURIComponent(payObj.ownerEmail)}`, {
      headers: { 'Authorization': 'Bearer ' + token }
    });
    if (res.ok) {
      const usersArr = await res.json();
      userObj = Array.isArray(usersArr) ? usersArr[0] || {} : usersArr || {};
    }
  } catch (e) { userObj = {}; }
...
// Получить список платежей с сервера
